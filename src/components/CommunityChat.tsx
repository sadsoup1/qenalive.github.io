import { Box, Flex, Text, IconButton, Drawer, DrawerOverlay, DrawerCloseButton, DrawerContent, DrawerBody, Avatar, InputGroup, Input, InputRightElement, Button, useToast, useColorModeValue } from '@chakra-ui/react';
...
export default function Chat() {
    ...
    async function fetchMessages() {
        ...
        const { data, error } = await supabase
            .from('course')
            .select('courseid')
            .eq('name', location.pathname.split('/')[2]);
        ...
        const communityId = data[0]?.courseid || '';
    ...
    async function getSingleAvatarUrl(userId: string) {
        const { data: user, error } = await supabase.from('user_profile').select('profile_pic').eq('userid', userId).single();
        if (error) {
            console.log(error);
        } else {
    ...
    }, [currentChatMessages, messages, myChatId]);
